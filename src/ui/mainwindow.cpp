...
void MainWindow::onPushButtonClicked()
{
    ...
    // STL dosyasını oku
    if (m_stlReader.readSTL(fileName, m_currentMesh)) {
        // Başarılı
        ...
        if (m_currentMesh.triangleCount() > 0) {
            Triangle& tri = m_currentMesh.triangles[0];
            qDebug() << "İlk üçgen:";
            qDebug() << "  Normal:" << tri.normal;
            qDebug() << "  Vertex1:" << tri.vertex1;
            qDebug() << "  Vertex2:" << tri.vertex2;
            qDebug() << "  Vertex3:" << tri.vertex3;
        }
    } else {
        // Hata
        QString error = "✗ STL dosyası okunamadı!\n\n" + m_stlReader.getLastError();
